    ...
    with httpx.Client(headers=headers) as client:
        ...
        for i in range(6):
            ...
            if response.status_code == 200:
                print(f"Work step {i+1} response:", response.text)
            time.sleep(3)

        # 获取奖励
        data = {"act": "getcre"}
        response = client.post("https://www.tsdm39.com/plugin.php?id=np_cliworkdz:work", headers=headers, data=data)
        print("Get reward response:", response.text)
...
def run_checkin():
    tsdm_check_in()
    score = get_score()
    print(f"Check-in completed. Current coins: {score}")

def run_work():
    tsdm_work()
    score = get_score()
    print(f"Work completed. Current coins: {score}")
